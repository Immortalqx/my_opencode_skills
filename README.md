...
| [`drawio-diagram`](./drawio-diagram/) | Draw.io research-figure workflow that builds an editable `.drawio` draft, exports PNG with a 60s hard timeout, and runs visual QA on the exported PNG. | Paper figures, posters, slide visuals, and concept diagrams that need an editable draw.io artifact. |
...
| [`mmx-cli`](./mmx-cli/) | MiniMax CLI skill for operating the local `mmx` command for search, vision, quota, file, and media tasks. `mmx text chat` is disabled. | Running an installed local MiniMax CLI directly, especially for bilingual multi-query search and non-interactive JSON workflows. |
