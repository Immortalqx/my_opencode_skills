#!/usr/bin/env python3
"""
Research Wiki helper utilities.
Canonical helper for the standalone /research-wiki skill. It handles wiki
initialization, paper ingest, edge updates, index rebuilds, compact query-pack
generation, and simple statistics.

Usage:
    python research_wiki.py init <wiki_root>
    python research_wiki.py slug "<paper title>" --author "<last name>" --year 2025
    python research_wiki.py add_edge <wiki_root> --from <node_id> --to <node_id> --type <edge_type> --evidence "<text>"
    python research_wiki.py rebuild_query_pack <wiki_root> [--max-chars 8000]
    python research_wiki.py rebuild_index <wiki_root>
    python research_wiki.py stats <wiki_root>
    python research_wiki.py log <wiki_root> "<message>"

    # Canonical paper ingest:
    python research_wiki.py ingest_paper <wiki_root> --arxiv-id <id> \
        [--thesis "<one-line>"] [--tags tag1,tag2] [--update-on-exist]

    # Manual ingest when arXiv metadata is not available:
    python research_wiki.py ingest_paper <wiki_root> \
        --title "<full title>" --authors "A, B, C" --year 2025 \
        --venue <venue> [--external-id-doi <doi>] [--thesis "..."] [--tags ...]

    # Batch backfill:
    python research_wiki.py sync <wiki_root> --arxiv-ids id1,id2,id3
    python research_wiki.py sync <wiki_root> --from-file ids.txt
"""

import sys as _codex_sys

for _codex_stream in (_codex_sys.stdout, _codex_sys.stderr):
    if hasattr(_codex_stream, "reconfigure"):
        _codex_stream.reconfigure(encoding="utf-8", errors="replace")
del _codex_stream
import argparse
import json
import os
    ...
    for f in ["index.md", "log.md", "gap_map.md", "query_pack.md"]:
        ...
        if not path.exists():
            if f == "index.md":
                ...
                path.write_text("# Query Pack\n\n_Auto-generated compact project summary. Max 8000 chars._\n")
...
def rebuild_query_pack(wiki_root: str, max_chars: int = 8000):
    """Generate a compressed query_pack.md for later reuse."""
    root = Path(wiki_root)
    sections = []
...
def ingest_paper(wiki_root: str, *, arxiv_id: str = "", title: str = "",
    ...
    if arxiv_id:
        ...
        if existing and not update_on_exist:
            # Log repeated scripted invocations so repeated no-op ingests are
            # still visible in the wiki history.
...
def main():
    parser = argparse.ArgumentParser(description="Research Wiki utilities")
    ...
    # ingest_paper — the canonical paper ingest entrypoint
