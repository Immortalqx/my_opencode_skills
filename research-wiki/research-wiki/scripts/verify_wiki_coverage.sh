...
#
# Compares arXiv IDs referenced in common local project artifacts against those
# already ingested into `research-wiki/papers/`. Reports the delta so the user
# can backfill missing papers with `sync`.
#
# This is best-effort and intentionally non-blocking.
#
# Usage:
#   bash scripts/verify_wiki_coverage.sh <wiki_root> [--scan <path> ...] [--json-out <path>]
#
# Defaults:
#   --scan: scans CWD-level `papers/`, `literature/`, `paper/`,
#           `PAPER_PLAN.md`, `NARRATIVE_REPORT.md`, `references.bib` if they exist.
...
# Default scan set: obvious local places where arXiv IDs show up
if [[ ${#SCAN_PATHS[@]} -eq 0 ]]; then
    for p in papers literature paper PAPER_PLAN.md NARRATIVE_REPORT.md references.bib; do
        [[ -e "$p" ]] && SCAN_PATHS+=("$p")
    done
fi
...
if [[ "$MISS_COUNT" -gt 0 ]]; then
    ...
    echo "    python3 scripts/research_wiki.py sync $WIKI_ROOT --arxiv-ids $HINT_IDS" >&2
